...
- **日记本**：写作私密日记，记录天气、心情指数和**多个情绪标签**，并用时间轴回顾；`MoodCard` 同时服务情绪记录和日记页。保存新日记时，若当天还没有情绪记录，会自动连同指数、标签和日记备注生成一条；当天已有记录（手动维护过）则不覆盖。编辑日记只同步由这篇日记自动生成的情绪记录，手动创建或事后手动改过的记录保持不变；删除日记后情绪记录仍然保留。时间轴会标注每篇日记是否关联了情绪记录。
